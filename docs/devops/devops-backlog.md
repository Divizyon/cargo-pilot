...
| **Y-08 · MinIO volume format migrasyonu** | 2026-08-18: #1065'in MinIO yükseltmesi (`2022-01-08` → `2024-01-31`) test sunucusunu düşürdü — mevcut volume eski **tek-disk FS formatında**, 2022-10 sonrası sürümler bu formatı okumuyor, container açılışta çöküyor (test-deploy run `32183514573`; aynı sorun daha önce de yaşanmıştı). Sürüm geri alındı, Dependabot'a `quay.io/minio/minio` ignore kuralı kondu. **Açık CVE'ler bilinçli taşınıyor:** CVE-2023-28432/28434, CVE-2024-24747, CVE-2022-31028. Kalıcı çözüm: eski sürüm ayaktayken yeni formatlı ikinci instance'a `mc mirror` ile veri taşıma (test + prod volume'leri), sonra sürüm yükseltme ve ignore kuralının kaldırılması. D-03 (MinIO yedeği) ile birlikte planlanmalı — migrasyon öncesi yedek şart | 🟠 Yüksek |
