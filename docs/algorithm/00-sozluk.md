...
| **BR1-BR7** | Bischoff & Ratcliff'in 1995 kıyas kümeleri (`data/br1..7.txt`). 7 küme × 100 örnek = **700 örnek**. Merdivenin tamamı elimizde: **BR0** tek tipli, **BR8-BR15** 30-100 tipli (Davies & Bischoff 1999) — ikisi de varsayılan koşuya girmez, baş sayı literatürle kıyaslanabilir kalsın diye (`DR-50`). Konteyner hepsinde 587×233×220 cm. Fark **kutu tipi sayısındadır**: BR1'de 3 tip, BR7'de 20 tip. Literatürde standart olduğu için sayımız başkalarınınkiyle kıyaslanabilir. |
...
| **strict** | BR "hangi ölçü dikey durabilir" kısıtı taşır ve üç düzende gelir: `001`, `011`, `111`. Üçü de artık `AllowedRotations`'a **birebir** eşleniyor (`DR-42`), dolayısıyla tek bir resmî sayı var. *Tarihsel not:* `011` düzeni bir süre `PitchOnly`'ye düşürüldü ve dört yasal yönelimin ikisi kayboldu; o dönemde sayı `strict` (alt sınır) / `free` (üst sınır) diye iki uçla verilirdi. `DR-20` bu yüzden konusuz kaldı — `free` ucu bugün fiziksel olarak yasak yerleşimleri sayar ve kaldırılmıştır. |
