...
| ~~DR-48~~ | ~~**Fazla kutu doluluğu düşürüyor — tekdüzelik bozuk**~~ — **çözüldü (`DR-56`, beam)** | Yerel görsel test düzeneği ortaya çıkardı: aynı araca artan yük verildiğinde doluluk düşüyor. A aracı, GRASP: %90 hedefte **%81,0**, %100 hedefte **%76,6**. Static'te de aynı yön (%71,1 → %67,7 @ %115). C aracında daha sert: %66,1 → %61,1. Fazladan kutu en kötü ihtimalle yerleşmeden kalmalı; elde daha çok seçenek varken sonucun kötüleşmesi için sebep yok. Yerleştirici tek geçişli ve geri dönüşsüz — sıradaki kutu yanlış yeri kapatıyor. **BR'de görünmedi** çünkü BR örneklerinin kutu sayısı sabit ve hepsi yaklaşık tam yük; "yük artarsa ne oluyor" sorusu hiç sorulmamıştı. **Kapanış:** ileri bakışlı ışın araması kusuru çözdü — beam her iki araçta da tekdüze (yük arttıkça doluluk hep artıyor), GRASP ikisinde de kırılıyor. Fark BR'dekinden **çok daha büyük**: BR1-BR7'de +1,08 puan, C aracında %115 yükte **+13,0 puan** (%65,1 → %78,1). Beam kararı sonuna kadar götürüp ölçtüğü için yanlış kapatmanın bedelini karar öncesi görüyor |
...
| **DR-55** | **Beam'in aksiyon uzayı "sıradaki ürün" yapıldı; GRASP ilk kez geçildi** | `DR-54`'ün teşhisi doğrulandı: iskelet doğruydu, dallanma uzayı kabaydı. Tek değişiklikle dallanma "hangi ayar"dan **"sıradaki ürün hangisi"**ye çevrildi — yerleştirici çevresine aynı üründen blok ördüğü için (`RaiseBlock`) bu fiilen *"bu boşluğa hangi blok"* sorusudur, kataloğu ayrıca bağlamaya gerek kalmadı. Aynı iskelet, aynı bütçe, **+2,2 puan** (%87,26 → %89,46). 175 örnekte **%89,42 vs GRASP %88,34 — her kümede kazanıyor**, en çok BR1'de (+1,96), ki `DR-51`'e göre blok zenginliğinin en yüksek olduğu küme. **F7-4 kapısı (≥ %89) geçildi.** `consumed[]` konuma bağlı olduğu için `PlacementState.WithConsumed` ile yeniden eşleniyor. Beam taban çizgisinin altına inemez (`best` statik koşuyla başlar). 173 test yeşil, statik yol bit birebir, 17 snapshot sabit. ⚠ **Üretim varsayılanı henüz değişmedi** — GRASP'tan geçiş kendi doğrulamasını ister |
| **DR-56** | **Üretim varsayılanı GRASP'tan beam'e geçti** (`DR-24` yenilendi) | 175 örnekte %88,34 → **%89,40**, yedi kümenin yedisinde de önde. Geçişten önce üç iş kapatıldı: (1) beam'in ayarları ölçülüp sabitlendi — `SearchBudget`'in `PopulationSize`/`MaxIterations` alanları GRASP için ayarlıydı (20/100) ve beam'in **en kötü** bölgesine denk geliyordu; artık yalnız süre paylaşılıyor. (2) `SearchStats` eklendi (`DR-26`). (3) Süre taşması düzeltildi: kontrol yalnız en içteki döngüdeydi, bir seviye 96 dal koşuyordu ve 2 sn'lik bütçe üretimde 4,52 sn'ye taşıyordu. **Gecikme regresyonu yok:** aynı istekte GRASP 4,09 sn, beam 4,06 sn. **Yan bulgu:** static de 2,09 sn sürüyor — isteğin yarısı motor değil, API/DB yükü. GRASP silinmedi, kıyas referansı olarak duruyor; CI kapısı statik yolu ölçmeye devam ediyor |
| **DR-57** | **F6-3 (duvar yüzü 2B tam kaplama) kapsam dışı; `DepthSlack` üretimde ama LIFO'ya yeniliyor** | *F6-3:* beam üretime girince ölçü tekrarlandı — doluluk arttıkça duvar kaplaması **düşüyor** (static %83,40/%85,8 → beam %89,40/%74,6). Yüz kaplaması doluluk kalitesinin değil **duvar disiplininin** göstergesiymiş ve arama onu bilerek feda ediyor. Duvar disiplini ürün gereksinimi de değil (`DR-45` kapandı, `DR-12`'nin gerçek gerekçesi kısmi doluluk biçimi). Dayanak kalmadı. *`DepthSlack`:* dört pay değerinde de doluluk birebir aynı (static %83,40, beam %89,45), yarım yükte derinlik %72,1 → %67,7; varsayılan `1,05` yapıldı. **Ama açar açmaz dört LIFO testi kırıldı** — hedef derinlik yükü öne toplarken LIFO grupları `z` boyunca yayıyor ve arkadaki grubun bandı kesiliyor (`DR-40`'ın hatası başka yoldan). **Öncelik kararı: LIFO varken hedef derinlik uygulanmaz** — boşaltma sırası iş kuralı, yoğunlaştırma tercih |
| **DR-58** | **`L(b)` knapsack kayıp terimi reddedildi; kaba sürüm daha iyi çalışıyor** | 19 Ağu araştırma yanıtının **birinci** önerisiydi, eşiği tek geçişli taban %83,4 → ≥%86. Terim doğru biçime çevrildi (sınırsız alt-küme toplamı; plan başına tek tablo, sorgu `O(1)`). **Eşik tutmadı:** static %83,40 → %83,41, ve üretim yolunda **beam %89,40 → %89,28** (iki koşu tutarlı, gürültü ±0,02). Kayıp üsteli de tarandı (1-8), düz. Geri alındı. **Sebep:** kaba sürüm ikili bir sinyaldi (*"dar artık = tam kayıp, değilse bedava"*) ve ince dilim üretmeye karşı **sert** bir baskıydı; `L(b)` aynı durumu sürekli küçük bir cezaya çevirip baskıyı sulandırıyor. Kaba terim `L(b)`'nin kötü bir yaklaşımı değil, motorumuza **daha iyi ayarlanmış farklı bir sezgiselmiş**. **Ders:** "literatürde bu terim var, demek ki bizde de kazandırır" çıkarımı güvenilir değil; sıradaki `CS(b)` sınaması için eşik temkinli konmalı |
| **DR-59** | **`CS(b)` temas yüzeyi eklendi: beam %89,40 → %89,95** | `L(b)` elenince araştırmanın greedy-taban tezinden geriye kalan terim. Bizde temas olarak yalnız **taban ayak izi + araç yüzeyleri** sayılıyordu; **komşu kutulara değme hiç sayılmıyordu**. `TopDeviation` zaten yerleşimleri tarayıp komşuları buluyordu, temas **alanı** aynı taramadan çıktı — **ek maliyet yok** (`Neighbourhood`). Static %83,40 → **%83,63**, beam %89,40 → **%89,95** (tekrar %89,96). Temas üsteli tarandı (0,25-1,5), düz; `0,5` korundu. **`L(b)`'nin tersine çalışmasının sebebi:** `L(b)` var olan bir sinyali sulandırıyordu, `CS(b)` **hiç olmayan** bir sinyali ekliyor — "bu kutu komşularına ne kadar yaslanıyor". Yaslanan yerleşim katı platform üretir; `DR-47`'nin "yığın yukarı çıkamıyor" teşhisinin doğrudan karşılığı. Kazancın beam'de static'ten büyük olması araştırmanın *"beam kalitesi greedy tabana bağlıdır"* tezini doğruluyor |
| **DR-60** | **İteratif ışın genişletme: %89,95 → %90,04, hedef bandına girildi** | Sabit genişlik-8 kolay kümelerde bütçenin dörtte birini kullanıyordu (BR1: 559/2000 ms). Arama artık `D=1`'den başlayıp her turda genişliği ikiye katlayarak süre bitene kadar koşuyor; en iyi turlar boyunca saklanıyor (Libralesso & Fontan 2020). Geometrik büyüme tekrar maliyetini son turun ~2 katında tutuyor. Kazanç küçük (+0,09, tekrar %90,03) ama **her küme bütçeyi dolduruyor** ve en çok boşa giden kümelerde kazanıldı (BR1 +0,21, BR7 +0,12). **Asıl fayda yapısal:** arama artık **anytime** — süre dolunca elde her zaman tamamlanmış bir tur var. Araştırmanın %90-92 hedef bandına girildi. ⚠ **Puan iddiası geri çekildi (`DR-61`):** +0,09 beam gürültü bandının (~±0,05) içinde. Kararın gerekçesi doluluk değil **yapısaldır** — arama anytime oldu ve her küme bütçeyi dolduruyor; o gerekçe ayakta |
| **DR-61** | **(blok, boşluk) aksiyon uzayı reddedildi; beam gürültü bandı ~±0,05 olarak düzeltildi** | `DecoderKeys.CandidateRank` ile arama ikinci/üçüncü en iyi adayı da deneyebiliyordu — dallanma fiilen *(blok, boşluk)* çiftine çıkıyordu (BSG'nin aksiyon uzayı). `rank = 0` ile statik yol bit birebir korundu. **Hiçbir bütçede kazandırmadı:** 2 sn %90,04 → %90,04 (rank 0,1) / %89,98 (rank 0,1,2); 8 sn %90,69 → %90,64. Geri alındı. **Sebep:** VCS'in ikinci en iyi adayı yapısal olarak farklı bir karar değil, aynı boşlukta komşu bir yönelim — beklenen çeşitlilik yok, buna karşılık varyant sayısı artıp tur sayısı düşüyor. **Yan bulgu ve önemli düzeltme:** geri alma sonrası aynı kod üç kez %89,98/%89,98/%89,97 verdi, oysa saatler önce %90,04/%90,03 vermişti — makine yükü (beş Docker konteyneri). Beam'in bütçesi duvar saati olduğu için **gürültü bandı ±0,01 değil ~±0,05**; bundan sonra 0,1 puandan küçük beam farkları gürültü sayılır. Static etkilenmez (saf hesap) |
| **DR-62** | **Bileşik blok eşiği zaten optimum; F-4 kapandı** | Araştırmanın F-4'ü. Bileşik blok bizde zaten etkin (`TopUp`); eşik `FootprintMatch = 0,85` tarandı (static, 700 örnek, gürültüsüz): 0,60 %82,57 · 0,70 %83,12 · 0,80 %83,56 · 0,82 %83,59 · **0,85 %83,63** · 0,88 %83,56 · 0,90 %83,50 · 0,95 %83,29 · 1,00 %83,23. **Temiz tek tepeli eğri, tepe tam mevcut değerde.** Eğrinin şekli mekanizmanın çalıştığını da gösteriyor: 1,00'da fiilen kapanıyor ve 0,40 puan kaybediliyor — yani "bileşik blok GRASP'ta ±0" bulgusu beam'e taşınmıyor, mekanizma değerli, yalnız ek kalibrasyonda puan yok. **Yan tespit:** `BlockCatalog` üretim yolunda hiç çağrılmıyor (`F7-4b`'de dallanma "sıradaki ürün" olunca gerek kalmadı); ölü değil, ölçüm düzeneği kullanıyor ama üretim derlemesinde duruyor — açık borç |
| **DR-63** | **Doygunluk kümeye bağlı; F-5/F-6 sıraya alınmadı, F-3 öne geçti** | F-6 (VNS) ve F-5 (defragmentation) "planda hâlâ doldurulabilecek yer var" varsayımına dayanıyordu. Ölçüldü: yerleşemeyen kutuların %10,5'i bir boşluğa geometrik sığıyor ama **sığan+destekli %0,0** — kalan boşlukların tabanı havada (`DR-47`). Boşluk doldurmak mümkün değil. **Süre/kalite eğrisi iki rejim gösterdi** (8 örnek, 2 sn → 30 sn): BR1 %89,53 → %89,59 (**+0,06, doymuş**), BR3 +0,29, BR5 **+1,24**, BR7 %89,35 → **%90,55 (+1,20, doymamış)**. Az tipli yükte dallanma uzayı küçük, arama tüketiyor, sınır **yapısal**; çok tipli yükte uzay büyük, 2 sn yetmiyor, sınır **verim**. **Sonuç:** çok tipli yük için **F-3 (rollout maliyetini düşür)** öne geçti — saniyede daha çok dal doğrudan doluluğa dönüyor. Az tipli yükte hiçbir arama iyileştirmesi işe yaramaz; orada kazanç ancak üretilen yüzeyin değişmesiyle gelir (`CS(b)` o kaldıracı bir kez çekti). F-5/F-6 ikisine de denk düşmüyor |
