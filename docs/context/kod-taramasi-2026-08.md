...
- **Sözleşme:** eksen eşlemesi ve köşe pivotu backend'de tutarlı; 2026-08-12'den beri yazılı da (`docs/COORDINATE_STANDARD.md`). Origin uzak-sol-alt köşedir, "sol-alt-arka" adlandırması geçersiz. cm birimi hiçbir yerde zorlanmıyor (konvansiyon). Pivot offset `scene-config.ts`'te değil, `BoxWrapper`/`CargoMeshInstanced` içine dağılmış (CLAUDE.md kuralının kısmi ihlali). Rotasyon→boyut eşlemesi FE/BE birebir uyumlu doğrulandı; ancak FE `ALLOWED_ROTATIONS.YawOnly=6`'nın backend enum karşılığı yok (latent uyumsuzluk). `allowContamination` FE'den gönderiliyor, BE komutlarında alan yok → sessizce yok sayılıyor.
