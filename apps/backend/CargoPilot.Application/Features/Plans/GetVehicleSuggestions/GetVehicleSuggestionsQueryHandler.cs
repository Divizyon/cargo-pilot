        ...
        CancellationToken cancellationToken)
    {
        ...
        var suggestions = vehicles
            .Select(v =>
            {
                ...
                var fillRate = vehicleVolume > 0
                    ? Math.Round(totalItemVolume / vehicleVolume * 100, 2)
                    : 0m;
                var canFitAll = fillRate <= 100 && totalItemWeight <= v.MaxWeightCapacity;
                return new VehicleSuggestionDto(v.Id, v.VehicleName, v.PlateNumber, v.VehicleType, fillRate, canFitAll);
            })
            .OrderByDescending(s => s.CanFitAll)
            .ThenByDescending(s => s.CanFitAll ? s.EstimatedFillRate : -s.EstimatedFillRate)
            .ToList();
