...
- `✅` Naming convention kurallarini genislet (Kapsam: `.editorconfig` kapsamli sekilde yeniden yazildi. Eklenen naming kurallari: (1) Interface `I` prefix + PascalCase — warning, (2) Generic tür parametresi `T` prefix + PascalCase — suggestion, (3) const/static readonly PascalCase (ALL_CAPS yasaklandi) — warning, (4) async method `Async` suffix — warning, (5) public/protected/internal uye PascalCase — warning, (6) local degisken ve parametre camelCase — suggestion. Mevcut private field `_camelCase` kurali korundu. Kod duzeni: `csharp_prefer_braces=true:warning` (tek satirlik if'de bile suslü parantez zorunlu), `csharp_style_namespace_declarations=file_scoped:warning` (C# 10+ file-scoped namespace), `csharp_using_directive_placement=outside_namespace:warning`, `dotnet_sort_system_directives_first=true`. Modern C# stil: `dotnet_style_require_accessibility_modifiers=warning` (erisim belirteci zorunlu), `this.` nitelendirmesi warning olarak yasaklandi (onceden suggestion), null-propagation/coalescing/pattern-matching suggestion, expression-body property/accessor/operator suggestion (constructor/method none). Tanisal override: IDE0055 suggestion. JSON/YAML/XML/props dosyalari icin 2-bosluk girintisi eklendi.)
- `✅` Analyzer paketlerini Directory.Build.props'a tasi (DRY) (Kapsam: `Microsoft.CodeAnalysis.NetAnalyzers` ve `SonarAnalyzer.CSharp` her csproj'dan kaldirildi; `Directory.Build.props`'a merkezi `ItemGroup` olarak tasindi. Ek duzeltme: `Infrastructure` projesinde `NetAnalyzers` eksikti, merkezi tasinimla bu bosluk kapatildi. `ImplicitUsings`, `Nullable`, `LangVersion` de Directory.Build.props'ta tanimli oldugundan csproj'lardaki tekrarlar silindi — her proje sadece kendine ozgu paket ve proje referanslarini icerecek sekilde temizlendi.)
