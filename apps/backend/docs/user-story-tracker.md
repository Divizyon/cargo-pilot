...
## 12) US-DASH-04: Plan Listesi ve Detay Endpoint'leri
**Story:** Backend ekibi olarak, Dashboard ve Plan Listesi ekranlarına veri sağlamak için yükleme planlarını sayfalı/sıralı listeleyen ve tek plan detayını tüm ilişkili verilerle döndüren performanslı endpoint'ler isterim.

**Genel Durum:** `✅ Tamamlandi`

### [BE-01] GET /api/v1/plans — Plan Listesi
- `✅` `GetPlansQuery` record'u tanımla (`Page`, `PageSize`, `SortBy`, `SortDirection` parametreleri, varsayılanlar: 1/20/createdAt/desc)
- `✅` `GetPlansQueryValidator` yaz (Page≥1, PageSize 1–100, SortBy whitelist, SortDirection {asc/desc}; FluentValidation; hata kodları: `PLAN_LIST_*`)
- `✅` `GetPlansQueryHandler` yaz (validation → ILoadingPlanRepository.GetPagedAsync)
- `✅` `PlanSummaryDto` tanımla (12 alan: Id, PlanName, OptimizationStatus, OptimizationCriteria, TotalWeight, FillRate, InputTotalQuantity, PlacedQuantity, UnplacedQuantity, VehicleId, VehicleName, CreatedAtUtc)
- `✅` `ILoadingPlanRepository.GetPagedAsync` interface metodu ekle
- `✅` `LoadingPlanRepository.GetPagedAsync` implemente et (AsNoTracking, IQueryable switch sort, Skip/Take, Select projection — EF Core JOIN ile VehicleName, Include yok, N+1 yok)
- `✅` `PlansController` oluştur; `[HttpGet]` endpoint ekle; FluentValidation 400, başarı 200
- `✅` `ILoadingPlanRepository` DI kaydını `DependencyInjection.cs`'e ekle

### [BE-02] GET /api/v1/plans/{id} — Plan Detayı
- `✅` `GetPlanByIdQuery` ve `GetPlanByIdQueryHandler` yaz (bulunamazsa `ErrorType.NotFound` / `Plan.NotFound`)
- `✅` Sub-DTO'ları tanımla: `ItemInPlanDto`, `VehicleInPlanDto`, `PlacementDto`, `UnplacedItemDto`, `WarningDto`
- `✅` `PlanDetailDto` tanımla (tüm plan alanları + ErrorCode?, ErrorMessage?, CenterOfGravity?, Vehicle, Placements, UnplacedItems, Warnings)
- `✅` `ILoadingPlanRepository.GetDetailByIdAsync` interface metodu ekle
- `✅` `LoadingPlanRepository.GetDetailByIdAsync` implemente et (4 ayrı odaklanmış sorgu: LoadingPlan+Vehicle, Placements+Item, UnplacedItems+Item, Warnings+RelatedItem+RelatedPlacement; `LoadingPlan` entity'sinde collection nav property olmadığı için bu mimari zorunluydu)
- `✅` `PlansController`'a `[HttpGet("{id:guid}")]` endpoint ekle; 200/404

### Güvenlik & Kalite
- `✅` Her iki endpoint sınıf düzeyi `[Authorize]` ile korunuyor; `[AllowAnonymous]` yok
- `✅` Build: 0 hata, 0 uyarı
- `✅` `appsettings.json` Jwt bloğu: `AccessTokenExpiryMinutes: 60`, `Secret` eklendi (dev ortamı 401 fix)

**Kanitlar:**
- `CargoPilot.Application/Features/Plans/GetPlans/PlanSummaryDto.cs`
- `CargoPilot.Application/Features/Plans/GetPlans/GetPlansQuery.cs`
- `CargoPilot.Application/Features/Plans/GetPlans/GetPlansQueryValidator.cs`
- `CargoPilot.Application/Features/Plans/GetPlans/GetPlansQueryHandler.cs`
- `CargoPilot.Application/Features/Plans/GetPlanById/PlanDetailDto.cs`
- `CargoPilot.Application/Features/Plans/GetPlanById/GetPlanByIdQuery.cs`
- `CargoPilot.Application/Features/Plans/GetPlanById/GetPlanByIdQueryHandler.cs`
- `CargoPilot.Application/Features/Plans/GetPlanById/ItemInPlanDto.cs`
- `CargoPilot.Application/Features/Plans/GetPlanById/VehicleInPlanDto.cs`
- `CargoPilot.Application/Features/Plans/GetPlanById/PlacementDto.cs`
- `CargoPilot.Application/Features/Plans/GetPlanById/UnplacedItemDto.cs`
- `CargoPilot.Application/Features/Plans/GetPlanById/WarningDto.cs`
...
- `CargoPilot.WebAPI/appsettings.json` (Jwt.Secret + AccessTokenExpiryMinutes)
