...
- `🟡` Development icin User Secrets kurulumunu yap (`UserSecretsId` ekle, kullanim komutlarini belgele) — **Backend takimi sorumlusudur** | **Sonra Yapılacak**: Secrets infrastructure'u tamamlandiktan sonra
- `🟡` Local gelistirme icin secret/config yukleme stratejisini belirle (User Secrets + `launchSettings.json` profilleri) — **Backend takimi sorumlusudur** | **Sonra Yapılacak**: Ortam bazli config stratejisi netlestirildiginde
- `⬜` `.env.example` olustur ve zorunlu/opsiyonel degiskenleri belgele — **DevOps takimi sorumlusudur**; infrastructure ortamlari (dev/staging/prod) basinda .env template'leri yonetir
- `⬜` `docs/environment-variables.md` icinde oncelik sirasi ve ortam bazli kaynak tablosu bolumunu genislet (DevOps ile koordinasyon) — **Simdilik Kismi Ertelendi**: Naming standardi ve naming ornekleri tamamlanmis; ortam bazli stratejiler (user-secrets / env var / appsettings / vault) DevOps'un infrastructure planlamasi tamamlandi sonra finalize edilecek
- `⬜` Gizli verilerin dosyalarda tutulmamasini garanti edecek policy ekle (dokumanda + `.gitignore` + code review kurali) — **DevOps/Security takimi sorumlusudur**; secret scanning ve access policy kurallarini yonetir
