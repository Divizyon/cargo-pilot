...
```ts
export const SCENE = {
  CAMERA_POSITION:[0,8,14] as const, CAMERA_FOV:50,
  ORBIT_MIN_DISTANCE:2, ORBIT_MAX_DISTANCE:50,
  LOAD_INTERVAL_MS:380, DROP_EASING:0.12,
  DROP_GLOW:0.25, IDLE_GLOW:0.06,
  COLORS:{VIOLATION:0xDC2626, SELECTED:0xfbbf24,
...
## Canvas
```tsx
<Canvas camera={{position:SCENE.CAMERA_POSITION,fov:SCENE.CAMERA_FOV}}
  ...
  <OrbitControls ref={orbitRef} enableDamping dampingFactor={0.05}
    ...
    minDistance={SCENE.ORBIT_MIN_DISTANCE} maxDistance={SCENE.ORBIT_MAX_DISTANCE}
...
meshes.forEach(m=>{
  ...
  if(Math.abs(d)>0.005){m.position.y+=d*SCENE.DROP_EASING;}
  else{m.position.y=m.userData.targetY;m.userData.active=false;
       m.material.emissiveIntensity=SCENE.IDLE_GLOW;}
});
// Cancel — plan değişince
useEffect(()=>{
  if(animationState==='loading'){cancelAnimation();clearScene();setAnimationState('idle');}
},[selectedPlanId]);
```
