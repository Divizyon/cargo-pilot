    ...
    <div className="h-full flex flex-col gap-3">
      ...
      <div className="bg-background rounded-xl border border-border overflow-hidden shrink-0 flex flex-col">
        ...
        <div className="px-3 pt-3 pb-3 border-t border-border shrink-0 flex flex-col gap-2">
          ...
          {/* Grup kümeleme modu */}
          <div className="flex flex-col gap-1">
            <span className={cn('text-[10px] px-0.5', criteria === OptimizationCriteria.Lifo ? 'text-muted-foreground/40' : 'text-muted-foreground')}>
              Grup Yerleştirme
              {criteria === OptimizationCriteria.Lifo && <span className="ml-1 text-[9px]">(LIFO ile kilitli)</span>}
            </span>
            <div className="grid grid-cols-2 gap-1">
              <button
                onClick={() => setClusterGroups(true)}
                disabled={criteria === OptimizationCriteria.Lifo}
                title={criteria === OptimizationCriteria.Lifo ? 'LIFO modunda grup kümeleme devre dışı' : 'Gruplu ürünler bir arada kümelenir'}
                className={cn(
                  'flex flex-col items-center gap-0.5 py-1.5 rounded-md text-[10px] border transition-colors',
              ...
              <button
                ...
                disabled={criteria === OptimizationCriteria.Lifo}
                title={criteria === OptimizationCriteria.Lifo ? 'LIFO modunda grup kümeleme devre dışı' : 'Tüm ürünler optimizasyon kriterine göre karışık yerleşir'}
                className={cn(
                  'flex flex-col items-center gap-0.5 py-1.5 rounded-md text-[10px] border transition-colors',
                  criteria === OptimizationCriteria.Lifo
                    ? 'opacity-40 cursor-not-allowed bg-background text-muted-foreground border-border'
